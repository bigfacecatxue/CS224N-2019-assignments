...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            modified = self.stack.pop(-2)
            head = self.stack[-1]
            self.dependencies.append((head, modified))
        else:
            modified = self.stack.pop(-1)
            head = self.stack[-1]
            self.dependencies.append((head, modified))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for partialparse in minibatch:
            if not partialparse.buffer and len(partialparse.stack) == 1:
                unfinished_parses.remove(partialparse)
    
    dependencies = [partialparse.dependencies for partialparse in partial_parses]
